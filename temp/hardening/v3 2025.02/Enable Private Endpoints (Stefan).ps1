...
If Private DNS zones are in a different subscription, specify the subscription id and resource group name in the
ExistingDNSZonesSubscriptionId and ExistingDNSZonesRG parameters. If the Private DNS zones are in a different
subscription, we assume that we will not be able to manipulate the links because the service account lacks the
permissions and these are managed centrally. However, the service account of Nerdio on the resource group in which
the Private DNS zones are located should grant the “Private DNS Zone Contributor” permission
to the service account “nerdio-nmw-app”.
...
if ($ExistingDNSZonesRG) {
    ...
    If ($ExistingDNSZonesSubscriptionId) {
        $PrivateDNSZoneContext = Set-AzContext -SubscriptionId $ExistingDNSZonesSubscriptionId

        # If the Private DNS Zones are in a different subscription, we assume that we will not be able to manipulate the links
        $linkDNSZones = $false
    }
    ...
    try {
        ...
        $CclKeyVaultDnzZone = Get-AzPrivateDnsZone -ResourceGroupName $DnsRg -Name privatelink.vaultcore.azure.net -ErrorAction Stop
        ...
        $ScriptedActionsDnsZone = Get-AzPrivateDnsZone -ResourceGroupName $DnsRg -Name privatelink.azure-automation.net -ErrorAction Stop
        $StorageDnsZone = Get-AzPrivateDnsZone -ResourceGroupName $DnsRg -Name privatelink.blob.core.windows.net -ErrorAction Stop
        $ScriptedActionsStorageDnsZone = Get-AzPrivateDnsZone -ResourceGroupName $DnsRg -Name privatelink.blob.core.windows.net -ErrorAction Stop
        $CclStorageDnsZone = Get-AzPrivateDnsZone -ResourceGroupName $DnsRg -Name privatelink.blob.core.windows.net -ErrorAction Stop
    ...
    If ($ExistingDNSZonesSubscriptionId) {
        $NMEContext = Set-AzContext -SubscriptionId $NMEContext.Subscription.Id
    }
    ...
    $CclKeyVaultDnzZone = Get-AzPrivateDnsZone -ResourceGroupName $DnsRg -Name privatelink.vaultcore.azure.net -ErrorAction SilentlyContinue
    ...
    $ScriptedActionsDnsZone = Get-AzPrivateDnsZone -ResourceGroupName $DnsRg -Name privatelink.azure-automation.net -ErrorAction SilentlyContinue
    ...
    $ScriptedActionsStorageDnsZone = Get-AzPrivateDnsZone -ResourceGroupName $DnsRg -Name privatelink.blob.core.windows.net -ErrorAction SilentlyContinue
    $CclStorageDnsZone = Get-AzPrivateDnsZone -ResourceGroupName $DnsRg -Name privatelink.blob.core.windows.net -ErrorAction SilentlyContinue
...
if ($VNet) {
    ...
    # Only on a new VNet the Private DNS Zones can be linked
    $newVNet = $true
}

#region create DNS zones and links
# Check if Private DNS Zones should be linked and if the VNet is new. If the VNet is not new and the Private DNS Zones are in a different subscription,
# we assume that we will not be able to manipulate the links
if ($linkDNSZones -and $newVNet) {
